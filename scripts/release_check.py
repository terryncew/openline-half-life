...
def main() -> int:
    checks: dict[str, object] = {"schema": "openline.half-life.release-check.v1", "version": "0.4.0rc4"}
    ...
    with tempfile.TemporaryDirectory(prefix="half-life-release-") as tmp:
        ...
        checks["wheel"] = {
            "passed": wheel.returncode == 0 and len(wheels) == 1 and install is not None and install.returncode == 0 and imported is not None and imported.returncode == 0 and imported.stdout.strip() == "0.4.0rc4" and installed_demo is not None and installed_demo.returncode == 0,
